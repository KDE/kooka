...
KScanOption::KScanOption(const QByteArray &name, KScanDevice *scandev)
{
    ...
    if (!mIsReadable) {
        return;    // no value to read
    }
    if (mBuffer.isNull()) {
        return;    // no buffer for value
    }
...
bool KScanOption::initOption(const QByteArray &name)
{
    ...
    if (mDesc->type == SANE_TYPE_GROUP) {
        mIsGroup = true;
    }
    if (mIsGroup || mDesc->type == SANE_TYPE_BUTTON) {
        mIsReadable = false;
    }
    if (!(mDesc->cap & SANE_CAP_SOFT_DETECT)) {
        mIsReadable = false;
    }
...
#ifdef DEBUG_MEM
    if (!mBuffer.isNull()) {
        //qDebug() << "Freeing" << mBuffer.size() << "bytes for" << mName;
    }
#endif
...
{
    if (!isValid() || !isReadable() || mControl == NULL || mBuffer.isNull()) {
        return;
    }
...
ret:
#ifdef DEBUG_APPLY
    //qDebug() << debug.constData();          // no quotes, please
#endif // DEBUG_APPLY
    return (reload);
}
...
KScanOption::WidgetType KScanOption::type() const
{
    ...
    if (isValid()) {
        ...
        case SANE_TYPE_FIXED:
            if (mDesc->constraint_type == SANE_CONSTRAINT_RANGE) {
                if (mDesc->unit == SANE_UNIT_DPI) {
                    ...
                    if (mDesc->size == sizeof(SANE_Word)) {
                        ret = KScanOption::Range;
                    } else {
        ...
        case SANE_TYPE_STRING:
            if (QString::compare(mDesc->name, SANE_NAME_FILE) == 0) {
                ret = KScanOption::File;
            } else if (mDesc->constraint_type == SANE_CONSTRAINT_STRING_LIST) {
                ret = KScanOption::StringList;
            } else {
                ret = KScanOption::String;
            }
            break;
        ...
        default:    //qDebug() << "unsupported SANE type" << mDesc->type;
            ret = KScanOption::Invalid;
            break;
        }
    }
...
#ifdef DEBUG_GETSET
    //qDebug() << "for SANE type" << (mDesc ? mDesc->type : -1) << "returning" << ret;
#endif
    return (ret);
}
...
bool KScanOption::set(const KGammaTable *gt)
{
    ...
    case SANE_TYPE_INT:
        for (int i = 0; i < size; ++i) {
            qa[i] = static_cast<SANE_Word>(run[i]);
        }
        break;

    case SANE_TYPE_FIXED:
        for (int i = 0; i < size; ++i) {
            qa[i] = SANE_FIX(static_cast<double>(run[i]));
        }
        break;
...
QByteArray KScanOption::get() const
{
    ...
    if (type() == KScanOption::GammaTable) {
        if (mGammaTable != NULL) {
            retstr = mGammaTable->toString().toLocal8Bit();
        }
    } else {
        switch (mDesc->type) {
        case SANE_TYPE_BOOL:
...
bool KScanOption::getRange(double *minp, double *maxp, double *quantp) const
{
    ...
    if (mDesc->constraint_type == SANE_CONSTRAINT_RANGE) {
        ...
        if (num >= 2) {
            quant = (max - min) / (num - 1);    // synthesise from total range
        }
    } else {
...
KScanControl *KScanOption::createWidget(QWidget *parent)
{
    ...
    if (mDesc != NULL) {
        mText = i18n(mDesc->title);
    }

    //qDebug() << "type" << type() << "text" << mText;
    ...
    default:
        //qDebug() << "unknown control type " << type();
        break;
    }
...
QLabel *KScanOption::getLabel(QWidget *parent, bool alwaysBuddy) const
{
    ...
    if (isCommonOption() || alwaysBuddy) {
        l->setBuddy(mControl->focusProxy());
    }
    return (l);
}
