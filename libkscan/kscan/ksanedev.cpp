...
/* KSANE Option                                                             */
/* ************************************************************************ */
KSANEOption::KSANEOption( const char *new_name ) :
   QObject()
{
	initOption( new_name );
...
void KSANEOption::initOption( const char *new_name )
{
   desc = 0;
   if( ! new_name ) return;
...
KSANEOption::KSANEOption( const KSANEOption &so ) :
   QObject()
{
...
const KSANEOption& KSANEOption::operator= (const KSANEOption& so )
{
   /* desc is stored by sane-lib and may be copied */
...
void KSANEOption::slWidgetChange( const char *t )
{
...
void KSANEOption::slWidgetChange( void )
{
...
void KSANEOption::slWidgetChange( int i )
{
 ...
 * this are equal !
 */
void KSANEOption::slRedrawWidget( KSANEOption *so )
{
	...
	if( so->valid() && w && so->getBuffer() )
	{
	 	switch( so->type( ) )
 		{
 	 		...
 	 		case RANGE:
 	 	  		...
 	 	  		if( so->get( &help ))
 	 	  		((KSANESlider*)w)->slSetSlider( help );
 	 		...
 	 		case STR_LIST:	 		
				...
				((KSANECombo*)w)->slSetEntry( so->get() ); 	  		
 	 	  		/* Widget Type is Selection Box */
 	 		break;
 	 		case STRING:
 	 	   	 // w = entryField( parent, text );
				((KSANEEntry*)w)->slSetEntry( so->get() ); 	  	 	
 	  	 		/* Widget Type is Selection Box */
 	    	        break;
 	  		default:
 	  			// w  = 0;
...
void KSANEOption::slReload( void )
{
	int  *num = option_dic[ getName() ];	
...
bool KSANEOption::valid( void )
{
	return( desc && 1 );
}

bool KSANEOption::autoSetable( void )
{
	/* Refresh description */
	desc = getOptionDesc( name );
...
bool KSANEOption::softwareSetable( void )
{
	/* Refresh description */
	desc = getOptionDesc( name );
...
KSANE_Type KSANEOption::type( void )
{
...
bool KSANEOption::set( double val )
{
   if( ! desc ) return( false );
   bool ret = false;
   int  word_size = 0;
...
bool KSANEOption::set( int *val, int size )
{
...
bool KSANEOption::set( const char *val )
{
 	return( set( QString(val) ));
}

bool KSANEOption::set( QString strval )
{
   bool ret = false;
...
bool KSANEOption::set( KGammaTable *gt )
{
   if( ! desc ) return( false );
   bool ret = true;
...
const QString KSANEOption::get( void )
{
...
bool KSANEOption::get( KGammaTable *gt )
{
    if( gt )
    {
...
bool KSANEOption::getRange( double *min, double *max, double *q )
{
   if( !desc ) return( false );
   bool ret = true;
                                    ...
                                    const char *tooltip )
{
	...
	QStrList list;
 	...
 	{
 		internal_widget = w;
 		connect( this, SIGNAL( optionChanged( KSANEOption*)),
 		               SLOT( slRedrawWidget( KSANEOption* )));
...
QWidget *KSANEOption::comboBox( QWidget *parent, const char *text )
{
 	QStrList list = getList();
 	
 	KSANECombo *cb = new KSANECombo( parent, text, list);
...
QWidget *KSANEOption::entryField( QWidget *parent, const char *text )
{
	KSANEEntry *ent = new KSANEEntry( parent, text );
...
QWidget *KSANEOption::KSaneSlider( QWidget *parent, const char *text )
{
 	double min, max, quant;
 	getRange( &min, &max, &quant );

	KSANESlider *slider = new KSANESlider( parent, text, min, max );
...
/* ---------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
bool KSANEDev::guiSetEnabled( QString name, bool state )
{
				      ...
				      const char *desc, const char *tooltip )
{
    ...
    if( so->valid() && so->softwareSetable())
    {
 	...
 	if( w )
       	    connect( so,   SIGNAL( optionChanged( KSANEOption* ) ),
       	             this, SLOT(   slOptChanged( KSANEOption* )));	
    }
    else
    {
 	if( !so->valid())
   ...
   : QObject( parent )
{
    ...
    connect( this, SIGNAL( sigScanFinished( KSANEStat )), SLOT( slScanFinished( KSANEStat )));

}


KSANEStat KSANEDev::openDevice( const char* backend )
{
   KSANEStat    stat      = KSANE_OK;
   SANE_Status 	sane_stat = SANE_STATUS_GOOD;

   if( ! backend ) return KSANE_ERR_PARAM;
...
QString KSANEDev::getScannerName()
{
...
KSANEStat KSANEDev::find_options()
{
   KSANEStat 	stat = KSANE_OK;
   SANE_Int 	n;
   SANE_Int 	opt;
...
QStrList KSANEDev::getAllOptions()
{
   return( option_list );
}

QStrList KSANEDev::getCommonOptions()
{
   QStrList com_opt;
...
QStrList KSANEDev::getAdvancedOptions()
{
   QStrList com_opt;
...
KSANEStat KSANEDev::apply( KSANEOption *opt )
{
   KSANEStat   stat = KSANE_OK;
...
bool  KSANEDev::optionExists( const char *name )
{
   if( ! name ) return( 0 );
...
void KSANEDev::slOptChanged( KSANEOption *opt )
{
...
void KSANEDev::slReloadAllBut( KSANEOption *not_opt )
{
        if( ! not_opt )
        {
	...
	for( KSANEOption *so = gui_elements.first(); so; so = gui_elements.next())
	{
	    if( so != not_opt )
	    {
	 	so->slReload();
...
KSANEStat KSANEDev::acquirePreview( bool forceGray, int dpi )
{
	KSANEOption *so = 0;
 	...
 	KSANEOption preview( SANE_NAME_PREVIEW );
 	preview.set( true );
 	apply( &preview );
 	
 	KSANEOption mode( SANE_NAME_SCAN_MODE );
	...
	apply( &res );
 	...
 	/* Start scanning */
 	KSANEStat stat = acquire_data( true );
...
void KSANEDev::prepareScan( void )
{
 ...
 **/
KSANEStat KSANEDev::acquire( QString filename )
{
    KSANEOption *so = 0;
 ...
 **/
KSANEStat KSANEDev::createNewImage( SANE_Parameters *p )
{
  if( !p ) return( KSANE_ERR_PARAM );
  KSANEStat       stat = KSANE_OK;
  ...
  else
  {
    ...
    debug( "KSANE supports only bit dephts 1 and 8 yet!" );
  }

  if( ! img ) stat = KSANE_ERR_MEMORY;
  return( stat );
}
...
void KSANEDev::slScanFinished( KSANEStat status )
{
  // clean up
...
void KSANEDev::doProcessABlock( void )
{
  int 		  val,i;
  QRgb      	  col, newCol;
