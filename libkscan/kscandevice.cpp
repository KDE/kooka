...
KScanOption *KScanDevice::getGuiElement(const QByteArray &name, QWidget *parent)
{
    ...
    if (so != NULL) {
        return (so);    // if so, just return that
    }
    ...
    if (so->isValid()) {                // option was created
        ...
        if (w != NULL) {
            w->setEnabled(so->isActive() && so->isSoftwareSettable());
        } else {
            //qDebug() << "no widget created for" << name;
        }
...
KScanDevice::Status KScanDevice::openDevice(const QByteArray &backend)
{
    ...
    // search for scanner
    if (ScanDevices::self()->deviceInfo(backend) == NULL) {
        return (KScanDevice::NoDevice);
    }
...
KScanDevice::Status KScanDevice::findOptions()
{
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (d == NULL) {
            continue;    // could not get descriptor
        }
        ...
        if (!name.isEmpty()) {              // must now have a name
            ...
            //qDebug() << "Invalid option" << i << "(no name and not a group)";
        }
    }
...
{
    if (mCreatedOptions.contains(name)) {
        return (name);
    }
...
// TODO: needed? will be done by acquireData()
    if (mScanningState == KScanDevice::ScanInProgress) {
        emit sigScanFinished(KScanDevice::Cancelled);
    }
...
{
    //qDebug() << "######################################################################";
    //qDebug() << "Scanner" << mScannerName;
    //qDebug() << "----------------------------------+----+----+----+----+----+----+----+";
    //qDebug() << " Option-Name                      |SSEL|HSEL|SDET|EMUL|AUTO|INAC|ADVA|";
    //qDebug() << "----------------------------------+----+----+----+----+----+----+----+";
    ...
    //qDebug() << "Preview-Switch is" << pso.get();
}
...
KScanDevice::Status KScanDevice::createNewImage(const SANE_Parameters *p)
{
    ...
    if (itype == ImageMetaInfo::BlackWhite) {   // Line art (bitmap)
        ...
        for (int i = 0; i < 256; i++) {
            mScanImage->setColor(i, qRgb(i, i, i));
        }
    }
...
{
    if (mSavedOptions != NULL) {
        mSavedOptions->clear();
    } else {
        mSavedOptions = new KScanOptSet("TempStore");
    }
    ...
    if (xres == NULL) {
        xres = getOption(SANE_NAME_SCAN_RESOLUTION, false);
    }
    if (xres != NULL) {
        ...
        if (dpi == 0) {             // preview DPI not specified
            ...
            preview_dpi = (int) min;
            if (preview_dpi < MIN_PREVIEW_DPI) {
                preview_dpi = MIN_PREVIEW_DPI;
            }
...
KScanDevice::Status KScanDevice::acquireScan(const QString &filename)
{
    if (filename.isEmpty()) {
        ...
        for (OptionHash::const_iterator it = mCreatedOptions.constBegin();
            ...
            if (so->isActive() && so->isSoftwareSettable()) {
                so->apply();
            }
        }
...
KScanDevice::Status KScanDevice::acquireData(bool isPreview)
{
    ...
    if (mImageInfo != NULL) {
        delete mImageInfo;    // start with this clean
    }
    mImageInfo = NULL;
    ...
    while (true) {                  // loop while frames available
        ...
        if (stat == KScanDevice::Ok && mScanningState == KScanDevice::ScanStarting) {
            ...
            if (stat == KScanDevice::Ok) {
                ...
                if (sane_set_io_mode(mScannerHandle, SANE_TRUE) == SANE_STATUS_GOOD) {
                    ...
                    //qDebug() << "not using socket notifier (sane_set_io_mode() failed)";
                }
            }
        }
        ...
        // more frames to do
        if (mScanningState == KScanDevice::ScanNextFrame) {
            continue;
        }
...
void KScanDevice::doProcessABlock()
{
    ...
    if (mScanningState == KScanDevice::ScanIdle) {
        return;    // scan finished, no more to do
    }
...
void KScanDevice::saveStartupConfig()
{
    if (mScannerName.isNull()) {
        return;    // do not save for no scanner
    }
...
void KScanDevice::loadOptionSet(const KScanOptSet *optSet)
{
    ...
    for (KScanOptSet::const_iterator it = optSet->constBegin();
        ...
        if (so == NULL) {
            continue;    // we don't have this option
        }

        so->set(it.value());

        if (!so->isInitialised()) {
            //qDebug() << "Option" << so->getName() << "is not initialised";
        } else if (!so->isActive()) {
            //qDebug() << "Option" << so->getName() << "is not active";
        } else {
            //qDebug() << "Option" << so->getName() << "set to" << so->get();
            so->apply();
        }
    }
}
...
{
    if (mScannerName.isNull()) {
        //qDebug() << "Skipping config write, no scanner name!";
        return;
    }

    //qDebug() << "Storing config" << key << "in group" << mScannerName;
